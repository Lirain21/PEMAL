...
def train(args, cl_model, auc_model, para_loss, para_function_auc_cl,  device, loader, optimizer, epoch): # para_contrastive_loss,
    cl_model.train()
    auc_model.train()
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        loss_2 = criterion(auc_pred_log, y)
        loss = loss_1 + loss_2 
    ...
    #     # plt.scatter(train_label, train_pred_2)
        
    #     plt.plot(train_label, train_label)
        
    #     plt.title('concat fin and descriptors_train_0.0005'+str(epoch))
    #     plt.savefig(str(epoch)+'concat_train_lr_0.0005_result.jpg')
    #     plt.show()
     
        
    # print(math.sqrt(abs(total_loss)/count))
    ...
    return total_loss_1/count, total_loss_2/count 



def eval(args, cl_model, auc_model, para_loss, para_function_auc_cl,  device, loader, epoch): # para_contrastive_loss,
    cl_model.eval()
    auc_model.eval()
    y_scores_1 = []
    y_scores_2 = []
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        with torch.no_grad():
            ...
            y_scores_2.append(auc_pred_log)
            
    y_scores_1 = torch.cat(y_scores_1, dim = 0)
    y_scores_2 = torch.cat(y_scores_2, dim = 0)
    y_true_value = torch.cat(y_true_value, dim = 0)
    
    loss = criterion(y_scores_1, y_true_value).cpu().detach().item() + criterion(y_scores_2, y_true_value).cpu().detach().item()
...
def main():
    ...
    parser.add_argument('--input_auc_model_file', type=str, default = 'results/atoms_0.75_motifs_0.1/auc/graphmae_lr_0.005_decay_1e-11_bz_64_seed_4_model.pt')
    ...
    for args.runseed in [2, 3, 4, 5]:
        ...
        for epoch in range(1, args.epochs+1):
            ...
            train_mes_loss = train(args, cl_model, auc_model, para_loss, para_function_auc_cl, device, train_loader, optimizer, epoch) # para_contrastive_loss,
            ...
            print("====Evaluation")
            if args.eval_train:
                train_mse_loss = eval(args, cl_model, auc_model, para_loss, para_function_auc_cl, device, train_loader, epoch) # para_contrastive_loss,
            else:
                print("omit the training accuracy computation")
                train_mse_loss = 0
            
            val_mse_loss = eval(args, cl_model, auc_model, para_loss, para_function_auc_cl,  device, val_loader, epoch) # para_contrastive_loss,

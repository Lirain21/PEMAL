...
def eval(args, cl_model, t1_2_model, vdss_model, para_loss, para_function_auc_cl, para_function_auc_vdss_t1_2,  device, loader): # para_contrastive_loss,
    ...
    y_scores_1 = torch.exp(y_scores_1)
    y_scores_2 = torch.exp(y_scores_2)
    y_true_value = torch.exp(y_true_value)
    
    all_result = torch.cat((y_scores_1, y_scores_2, y_true_value), dim=1).detach().cpu().numpy()
    np.savetxt("threetasks_test_result.csv", all_result, delimiter=",")
...
def main():
    ...
    parser.add_argument('--input_vdss_model_file', type=str, default = 'results/auc_models/vdss/threetasksfinetune/graphmae_lr_1e-05_decay_1e-09_bz_32_seed_5_model.pt', help='filename to read the model (if there is any)')
    parser.add_argument('--input_cl_model_file', type=str, default = 'results/auc_models/cl/threetasksfinetune/graphmae_lr_1e-05_decay_1e-09_bz_32_seed_5_model.pt', help='filename to read the model (if there is any)')
    parser.add_argument('--input_t1_2_model_file', type=str, default = 'results/auc_models/t1_2/threetasksfinetune/graphmae_lr_1e-05_decay_1e-09_bz_32_seed_5_model.pt', help='filename to read the model (if there is any)')

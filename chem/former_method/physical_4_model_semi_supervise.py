...
def train(args, vdss_model, t1_2_model, cl_model, auc_model, para_loss, para_function_cl_vdss_t1_2, para_function_auc_cl,  device, loader, optimizer, epoch, train_labeled_loader, train_unlabel_iterator): # para_contrastive_loss,
    vdss_model.train()
    t1_2_model.train()
    cl_model.train()
    auc_model.train()
    ...
    train_labeled_iterator = iter(train_labeled_loader)
    while True:
        try:
            labeled_batch = next(train_labeled_iterator)
            labeled_batch = labeled_batch.to(device)

            unlabeled_batch = next(train_unlabel_iterator)
            unlabeled_batch = unlabeled_batch.to(device)
        
            count += 1 
            vdss_pred_log_labeled = vdss_model(labeled_batch.x, labeled_batch.edge_index, labeled_batch.edge_attr, labeled_batch.batch, True)
            t1_2_pred_log_labeled = t1_2_model(labeled_batch.x, labeled_batch.edge_index, labeled_batch.edge_attr, labeled_batch.batch, True)
            cl_pred_log_labeled = cl_model(labeled_batch.x, labeled_batch.edge_index, labeled_batch.edge_attr, labeled_batch.batch, True)
            auc_pred_log_labeled = auc_model(labeled_batch.x, labeled_batch.edge_index, labeled_batch.edge_attr, labeled_batch.batch, True)
        
            vdss_pred_log_unlabeled = vdss_model(unlabeled_batch.x, unlabeled_batch.edge_index, unlabeled_batch.edge_attr, unlabeled_batch.batch, True)
            t1_2_pred_log_unlabeled = t1_2_model(unlabeled_batch.x, unlabeled_batch.edge_index, unlabeled_batch.edge_attr, unlabeled_batch.batch, True)
            cl_pred_log_unlabeled = cl_model(unlabeled_batch.x, unlabeled_batch.edge_index, unlabeled_batch.edge_attr, unlabeled_batch.batch, True)
            auc_pred_log_unlabeled = auc_model(unlabeled_batch.x, unlabeled_batch.edge_index, unlabeled_batch.edge_attr, unlabeled_batch.batch, True)

            vdss_pred_labeled = torch.exp(vdss_pred_log_labeled)
            t1_2_pred_labeled = torch.exp(t1_2_pred_log_labeled)
            cl_pred_labeled = torch.exp(cl_pred_log_labeled)
            auc_pred_labeled = torch.exp(auc_pred_log_labeled)
        
        
            vdss_pred_unlabeled = torch.exp(vdss_pred_log_unlabeled)
            t1_2_pred_unlabeled = torch.exp(t1_2_pred_log_unlabeled)
            cl_pred_unlabeled = torch.exp(cl_pred_log_unlabeled)
            auc_pred_unlabeled = torch.exp(auc_pred_log_unlabeled)
            
            vdss_pred = torch.cat((vdss_pred_labeled, vdss_pred_unlabeled), dim=0)
            cl_pred = torch.cat((cl_pred_labeled, cl_pred_unlabeled), dim=0)
            t1_2_pred = torch.cat((t1_2_pred_labeled, t1_2_pred_unlabeled), dim=0)
            auc_pred = torch.cat((auc_pred_labeled, auc_pred_unlabeled), dim=0)
            ...
            # auc_pred_labeled = k_1/cl_pred_labeled 
            # auc_pred_log_labeled = torch.log(auc_pred_labeled)
            
            auc_y = labeled_batch.auc_y.reshape(labeled_batch.auc_y.size(0), 1)
            cl_y = labeled_batch.cl_y.reshape(labeled_batch.cl_y.size(0), 1)
            vdss_y = labeled_batch.vdss_y.reshape(labeled_batch.vdss_y.size(0), 1)
            t1_2_y = labeled_batch.t1_2_y.reshape(labeled_batch.t1_2_y.size(0), 1)
            
            loss_auc = criterion(auc_pred_log_labeled, auc_y)
            loss_cl = criterion(cl_pred_log_labeled, cl_y)
            loss_vdss = criterion(vdss_pred_log_labeled, vdss_y)
            loss_t1_2 = criterion(t1_2_pred_log_labeled, t1_2_y)
            
            loss_k3 = criterion(torch.log(k_3 * vdss_pred), torch.log(t1_2_pred * cl_pred))
            loss_k1 = criterion(torch.log(k_1/cl_pred), torch.log(auc_pred))
            loss = loss_auc + loss_cl + loss_vdss + loss_t1_2 + loss_k3 + loss_k1 
            ...
            train_auc_pred_all.append(auc_pred_log_labeled)
            train_cl_pred_all.append(cl_pred_log_labeled)
            train_vdss_pred_all.append(vdss_pred_log_labeled)
            train_t1_2_pred_all.append(t1_2_pred_log_labeled)
...
def eval(args, vdss_model, t1_2_model, cl_model, auc_model,  para_loss, para_function_cl_vdss_t1_2, para_function_auc_cl,  device, loader, epoch): # para_contrastive_loss,
    vdss_model.eval()
    t1_2_model.eval()
    cl_model.eval()
    auc_model.eval()
...
def main():
    ...
    parser.add_argument('--unlabel_dataset', type=str, default="ic_50", help="root directory of dataset.")
    ...
    parser.add_argument('--input_auc_model_file', type=str, default='results/11_13/finetune/auc/graphmae_lr_0.0005_decay_1e-10_bz_64_seed_4_4model.pt', help='filename to read the model (if there is any)')
    ...
    for args.runseed in [1, 2, 3, 4, 5]:
        ...
        unlabel_dataset = MoleculeDataset("dataset_reg/"+args.unlabel_dataset, dataset=args.unlabel_dataset)
        
        print(train_dataset)
        print(valid_dataset)
        
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers = args.num_workers)
        unlabel_loader = DataLoader(unlabel_dataset, batch_size=args.batch_size, shuffle=True, num_workers = args.num_workers)
        ...
        if not args.input_auc_model_file == "":
            print('load pretrained model from:', args.input_auc_model_file)
        ...
        for epoch in range(1, args.epochs+1):
            ...
            train_unlabeled_iterator = iter(unlabel_loader)
            train_mes_loss, train_auc_pred, train_cl_pred, train_vdss_pred, train_t1_2_pred,\
            train_auc_label, train_cl_label, train_vdss_label, train_t1_2_label = train(args, vdss_model, t1_2_model, cl_model, auc_model, para_loss, para_function_cl_vdss_t1_2, para_function_auc_cl, device, train_loader, optimizer, epoch, train_loader, train_unlabeled_iterator) # para_contrastive_loss,
            ...
            auc_val_loss, cl_val_loss, vdss_val_loss, t1_2_val_loss, all_loss, \
                ...
                valid_auc_label, valid_cl_label, valid_vdss_label, valid_t1_2_label = eval(args, vdss_model, t1_2_model, cl_model, auc_model, para_loss, para_function_cl_vdss_t1_2, para_function_auc_cl, device, val_loader, epoch) # para_contrastive_loss,
    ...
    dataframe.to_csv("results/11_13/4_model/"+args.experiment_name+"_"+"result.csv", index=False)

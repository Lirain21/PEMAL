...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        elif self.dataset == "4_50_train" or "4_100_train" or "4_150_train" or "4_10_noise_train" or "4_20_noise_train" or "4_40_noise_train" or "4_60_noise_train" or "4_80_noise_train":
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            # label_list = list(input_df.auc_label)
            ...
            for i in range(len(smiles_list)):
                ...
                try:
                    ...
                    if rdkit_mol != None:
                        ...
                        data.id = torch.tensor([count])
                        # data.y = torch.tensor(label_list[i])
                        data.auc_y = torch.tensor(auc_label_list[i])
                        data.cl_y = torch.tensor(cl_label_list[i])
                        data.vdss_y = torch.tensor(vdss_label_list[i])
                        data.t1_2_y = torch.tensor(t1_2_label_list[i])
                        count += 1
                        print(count)
                        data_list.append(data)
                        data_smiles_list.append(smiles_list[i])
        ...
        elif self.dataset == "4_50_valid" or "4_100_valid" or "4_150_valid" or "4_10_noise_valid" or "4_20_noise_valid" or "4_40_noise_valid" or "4_60_noise_valid" or "4_80_noise_valid":
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            label_list = list(input_df.auc_label)
